...
contract PaniniValidator is Initializable,AccessControlUpgradeable {
    ...
    function _validateTransfer(address caller, address from, address to) internal view  {
        if (paniniLock) {
            ...
            if(!fromZeroAddress && !toZeroAddress && from != address(this)) {
                ...
                if (!hasRole(WHITELISTED_MARKETPLACE, caller) && caller != from) {
                    revert InvalidOperator("Caller Is Not Owner Or Whitelisted Marketplace ");
                }
            }
        }
    }
    ...
    function _validateTransfer(address caller, address from) internal view  {
        if (paniniLock) {
            ...
            if (!hasRole(WHITELISTED_MARKETPLACE, caller) && caller != from) {
                revert InvalidOperator("Caller Is Not Owner Whitelisted Marketplace ");
            }
